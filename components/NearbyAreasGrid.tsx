...
export function NearbyAreasGrid({ cityName, serviceSlug, serviceName, initialVisible = 10 }: NearbyAreasGridProps) {
  ...
  const description = serviceName
    ? `Looking for ${serviceName.toLowerCase()} near ${cityName}? Our vetted property management specialists serve landlords across ${cityName} and surrounding areas.`
    : `Our property management specialists serve landlords across the surrounding area. If your property is based in any of the areas listed below, we can match you with the right specialist.`;

  return (
    <section className="mb-16">
      <div className="flex items-center gap-3 mb-2">
        <div className="bg-brand-100 p-2 rounded-lg"><MapPin className="w-5 h-5 text-brand-600" /></div>
        <h2 className="text-2xl md:text-3xl font-display font-bold text-gray-900">{heading}</h2>
      </div>
      <p className="text-gray-600 mb-6 leading-relaxed">{description}</p>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
        {visibleAreas.map(area => (
          <div key={area} className="flex items-center gap-2 px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-lg text-sm text-gray-700">
            <MapPin className="w-3 h-3 text-brand-400 flex-shrink-0" /><span className="font-medium truncate">{area}</span>
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 prose prose-sm max-w-none text-gray-500">
        <p>Landlords from {areas.slice(0, 5).join(', ')}, and other areas around {cityName} regularly use our service to find property management specialists.{' '}
          {serviceName ? `If you need ${serviceName.toLowerCase()} and your property is in or near ${cityName}, our vetted specialists can arrange a free consultation.` : `All of our ${cityName} partner property managers are qualified, fully insured, and offer flexible appointment times.`}
        </p>
      </div>
    </section>
  );
